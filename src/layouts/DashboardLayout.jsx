...
export default function DashboardLayout({ children, user, onLogout, currentView, setCurrentView }) {
  ...
  return (
    <div data-theme={isDark ? 'dark' : 'light'} className={`h-screen w-full ${isDark ? 'bg-slate-900' : 'bg-slate-50'} flex font-sans overflow-hidden`}>
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-72 bg-slate-800 text-gray-100 shadow-xl border-r border-slate-700
        ...
        {/* Header del Sidebar */}
        <div className="flex items-center justify-between p-6 border-b border-slate-600 bg-slate-900/50 flex-shrink-0">
        ...
        <div className="border-t border-slate-600 p-4 bg-slate-900/30 flex-shrink-0">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        {/* Header móvil */}
        <header className={`md:hidden flex justify-between items-center ${isDark ? 'bg-slate-800/80 border-slate-700' : 'bg-white/80 border-gray-200'} backdrop-blur-lg p-4 shadow-lg border-b flex-shrink-0`}>
          <button
        ...
        <main className={`flex-1 overflow-y-auto ${isDark ? 'bg-slate-900' : 'bg-slate-50'} custom-scrollbar`}>
          <div className="p-6">
